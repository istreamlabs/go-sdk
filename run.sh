...
OPENAPI_SPEC_SLATES="http://api.istreamplanet.com/docs/slates/openapi.json"

ENV=$1
if [[ $ENV == "stage" ]]; then
  OPENAPI_SPEC="http://stage.api.istreamplanet.com/openapi.json"
  OPENAPI_SPEC_SLATES="http://stage.api.istreamplanet.com/docs/slates/openapi.json"
fi

echo -e "Generating SDK against ${OPENAPI_SPEC}\n"
echo -e "Generating Slate SDK against ${OPENAPI_SPEC_SLATES}\n"
...
docker build -t generate-sdk-slates . --no-cache --build-arg OPENAPI_SPEC="${OPENAPI_SPEC_SLATES}"
docker build -t generate-sdk . --no-cache --build-arg OPENAPI_SPEC="${OPENAPI_SPEC}"

docker run --rm -it -v ${SCRIPT_DIR}/isp:/go-sdk/isp generate-sdk-slates
