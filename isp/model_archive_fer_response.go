/*
 * WBD Aventus Channels API
 *
 * API version: 0.0.0
 * Contact: live-control-plane-devs@wbd.com
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package isp

import (
	"encoding/json"
)

// checks if the ArchiveFERResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ArchiveFERResponse{}

// ArchiveFERResponse struct for ArchiveFERResponse
type ArchiveFERResponse struct {
	// A URL to the JSON Schema for this object.
	Schema interface{} `json:"$schema,omitempty" format:"uri" doc:"A URL to the JSON Schema for this object."`
	// Identifier that is carried through the archive request
	CorrelationId interface{} `json:"correlation_id,omitempty" doc:"Identifier that is carried through the archive request"`
	// Task ID for the archive task started by the L2V service
	TaskId interface{} `json:"task_id" doc:"Task ID for the archive task started by the L2V service"`
}

// NewArchiveFERResponse instantiates a new ArchiveFERResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewArchiveFERResponse(taskId interface{}) *ArchiveFERResponse {
	this := ArchiveFERResponse{}
	this.TaskId = taskId
	return &this
}

// NewArchiveFERResponseWithDefaults instantiates a new ArchiveFERResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewArchiveFERResponseWithDefaults() *ArchiveFERResponse {
	this := ArchiveFERResponse{}
	return &this
}

// GetSchema returns the Schema field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ArchiveFERResponse) GetSchema() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.Schema
}

// GetSchemaOk returns a tuple with the Schema field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ArchiveFERResponse) GetSchemaOk() (*interface{}, bool) {
	if o == nil || IsNil(o.Schema) {
		return nil, false
	}
	return &o.Schema, true
}

// HasSchema returns a boolean if a field has been set.
func (o *ArchiveFERResponse) HasSchema() bool {
	if o != nil && IsNil(o.Schema) {
		return true
	}

	return false
}

// SetSchema gets a reference to the given interface{} and assigns it to the Schema field.
func (o *ArchiveFERResponse) SetSchema(v interface{}) {
	o.Schema = v
}

// GetCorrelationId returns the CorrelationId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ArchiveFERResponse) GetCorrelationId() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.CorrelationId
}

// GetCorrelationIdOk returns a tuple with the CorrelationId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ArchiveFERResponse) GetCorrelationIdOk() (*interface{}, bool) {
	if o == nil || IsNil(o.CorrelationId) {
		return nil, false
	}
	return &o.CorrelationId, true
}

// HasCorrelationId returns a boolean if a field has been set.
func (o *ArchiveFERResponse) HasCorrelationId() bool {
	if o != nil && IsNil(o.CorrelationId) {
		return true
	}

	return false
}

// SetCorrelationId gets a reference to the given interface{} and assigns it to the CorrelationId field.
func (o *ArchiveFERResponse) SetCorrelationId(v interface{}) {
	o.CorrelationId = v
}

// GetTaskId returns the TaskId field value
// If the value is explicit nil, the zero value for interface{} will be returned
func (o *ArchiveFERResponse) GetTaskId() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}

	return o.TaskId
}

// GetTaskIdOk returns a tuple with the TaskId field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ArchiveFERResponse) GetTaskIdOk() (*interface{}, bool) {
	if o == nil || IsNil(o.TaskId) {
		return nil, false
	}
	return &o.TaskId, true
}

// SetTaskId sets field value
func (o *ArchiveFERResponse) SetTaskId(v interface{}) {
	o.TaskId = v
}

func (o ArchiveFERResponse) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ArchiveFERResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.Schema != nil {
		toSerialize["$schema"] = o.Schema
	}
	if o.CorrelationId != nil {
		toSerialize["correlation_id"] = o.CorrelationId
	}
	if o.TaskId != nil {
		toSerialize["task_id"] = o.TaskId
	}
	return toSerialize, nil
}

type NullableArchiveFERResponse struct {
	value *ArchiveFERResponse
	isSet bool
}

func (v NullableArchiveFERResponse) Get() *ArchiveFERResponse {
	return v.value
}

func (v *NullableArchiveFERResponse) Set(val *ArchiveFERResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableArchiveFERResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableArchiveFERResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableArchiveFERResponse(val *ArchiveFERResponse) *NullableArchiveFERResponse {
	return &NullableArchiveFERResponse{value: val, isSet: true}
}

func (v NullableArchiveFERResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableArchiveFERResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


