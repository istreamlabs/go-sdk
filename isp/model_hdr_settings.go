/*
 * WBD Aventus Channels API
 *
 * API version: 0.0.0
 * Contact: live-control-plane-devs@wbd.com
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package isp

import (
	"encoding/json"
)

// checks if the HdrSettings type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &HdrSettings{}

// HdrSettings struct for HdrSettings
type HdrSettings struct {
	DolbyVision *H265SettingsHdrDolbyVision `json:"dolby_vision,omitempty"`
	Hdr10       *H265SettingsHdrHdr10       `json:"hdr10,omitempty"`
}

// NewHdrSettings instantiates a new HdrSettings object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewHdrSettings() *HdrSettings {
	this := HdrSettings{}
	return &this
}

// NewHdrSettingsWithDefaults instantiates a new HdrSettings object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewHdrSettingsWithDefaults() *HdrSettings {
	this := HdrSettings{}
	return &this
}

// GetDolbyVision returns the DolbyVision field value if set, zero value otherwise.
func (o *HdrSettings) GetDolbyVision() H265SettingsHdrDolbyVision {
	if o == nil || IsNil(o.DolbyVision) {
		var ret H265SettingsHdrDolbyVision
		return ret
	}
	return *o.DolbyVision
}

// GetDolbyVisionOk returns a tuple with the DolbyVision field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *HdrSettings) GetDolbyVisionOk() (*H265SettingsHdrDolbyVision, bool) {
	if o == nil || IsNil(o.DolbyVision) {
		return nil, false
	}
	return o.DolbyVision, true
}

// HasDolbyVision returns a boolean if a field has been set.
func (o *HdrSettings) HasDolbyVision() bool {
	if o != nil && !IsNil(o.DolbyVision) {
		return true
	}

	return false
}

// SetDolbyVision gets a reference to the given H265SettingsHdrDolbyVision and assigns it to the DolbyVision field.
func (o *HdrSettings) SetDolbyVision(v H265SettingsHdrDolbyVision) {
	o.DolbyVision = &v
}

// GetHdr10 returns the Hdr10 field value if set, zero value otherwise.
func (o *HdrSettings) GetHdr10() H265SettingsHdrHdr10 {
	if o == nil || IsNil(o.Hdr10) {
		var ret H265SettingsHdrHdr10
		return ret
	}
	return *o.Hdr10
}

// GetHdr10Ok returns a tuple with the Hdr10 field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *HdrSettings) GetHdr10Ok() (*H265SettingsHdrHdr10, bool) {
	if o == nil || IsNil(o.Hdr10) {
		return nil, false
	}
	return o.Hdr10, true
}

// HasHdr10 returns a boolean if a field has been set.
func (o *HdrSettings) HasHdr10() bool {
	if o != nil && !IsNil(o.Hdr10) {
		return true
	}

	return false
}

// SetHdr10 gets a reference to the given H265SettingsHdrHdr10 and assigns it to the Hdr10 field.
func (o *HdrSettings) SetHdr10(v H265SettingsHdrHdr10) {
	o.Hdr10 = &v
}

func (o HdrSettings) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o HdrSettings) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.DolbyVision) {
		toSerialize["dolby_vision"] = o.DolbyVision
	}
	if !IsNil(o.Hdr10) {
		toSerialize["hdr10"] = o.Hdr10
	}
	return toSerialize, nil
}

type NullableHdrSettings struct {
	value *HdrSettings
	isSet bool
}

func (v NullableHdrSettings) Get() *HdrSettings {
	return v.value
}

func (v *NullableHdrSettings) Set(val *HdrSettings) {
	v.value = val
	v.isSet = true
}

func (v NullableHdrSettings) IsSet() bool {
	return v.isSet
}

func (v *NullableHdrSettings) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableHdrSettings(val *HdrSettings) *NullableHdrSettings {
	return &NullableHdrSettings{value: val, isSet: true}
}

func (v NullableHdrSettings) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableHdrSettings) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
