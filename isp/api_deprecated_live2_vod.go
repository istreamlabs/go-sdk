...
func (r ApiDeprecatedClipGetPresentationsRequest) Execute() ([]DeprecatedGetPresentationsResponseItem, *http.Response, error) {
	return r.ApiService.DeprecatedClipGetPresentationsExecute(r)
}
...
//  @return []DeprecatedGetPresentationsResponseItem
// Deprecated
func (a *DeprecatedLive2VODApiService) DeprecatedClipGetPresentationsExecute(r ApiDeprecatedClipGetPresentationsRequest) ([]DeprecatedGetPresentationsResponseItem, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  []DeprecatedGetPresentationsResponseItem
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "DeprecatedLive2VODApiService.DeprecatedClipGetPresentations")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/cust/{customer-id}/prod/{product-id}/prog/{program-id}/vod/{vod_id}/clip/{clip-id}/pres"
	localVarPath = strings.Replace(localVarPath, "{"+"customer-id"+"}", url.PathEscape(parameterToString(r.customerId, "")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"product-id"+"}", url.PathEscape(parameterToString(r.productId, "")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"program-id"+"}", url.PathEscape(parameterToString(r.programId, "")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"vod_id"+"}", url.PathEscape(parameterToString(r.vodId, "")), -1)
	...
	if disablePaging := r.ctx.Value(ContextDisablePaging); disablePaging == nil {
		if uri := GetLink(localVarHTTPResponse, RelNext); uri != nil {
			...
			localVarReturnValue = items.([]DeprecatedGetPresentationsResponseItem)
			localVarHTTPResponse = resp
		}
	}
