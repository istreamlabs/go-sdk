/*
 * WBD Aventus Channels API
 *
 * API version: 0.0.0
 * Contact: live-control-plane-devs@wbd.com
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package isp

import (
	"encoding/json"
)

// checks if the Publication type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &Publication{}

// Publication struct for Publication
type Publication struct {
	// Optionally specify which audio encoders should be used for this publication. If none are specified, all audio encoders configured for the transcoder will be used.
	AudioEncoderIds interface{} `json:"audio_encoder_ids,omitempty" uniqueItems:"true" doc:"Optionally specify which audio encoders should be used for this publication. If none are specified, all audio encoders configured for the transcoder will be used."`
	// Create VODs for all publish points in this publication. Note that Live2VOD must also be configured for the parent |Channel|.
	CreateVods interface{} `json:"create_vods,omitempty" doc:"Create VODs for all publish points in this publication. Note that Live2VOD must also be configured for the parent |Channel|."`
	Dash *PublicationDash `json:"dash,omitempty"`
	// Optionally specify which DRMs to advertise in the playlist. If specified, this must be a subset of the DRMs specified by the packager associated with this publication. If omitted or empty, all DRMs specified by the packager will be advertised. This setting can only be used for HLS playlists.
	Drms interface{} `json:"drms,omitempty" uniqueItems:"true" doc:"Optionally specify which DRMs to advertise in the playlist. If specified, this must be a subset of the DRMs specified by the packager associated with this publication. If omitted or empty, all DRMs specified by the packager will be advertised. This setting can only be used for HLS playlists."`
	// DVR window is the max sum(duration of media segments) that will be kept in a manifest at a given time in seconds. The max supported DVR window is 10 hours.
	DvrWindowSecs interface{} `json:"dvr_window_secs,omitempty" format:"int32" minimum:"0" maximum:"36000" doc:"DVR window is the max sum(duration of media segments) that will be kept in a manifest at a given time in seconds. The max supported DVR window is 10 hours."`
	// Optionally specify which audio encoders should be used when generating the FER of this Presentation, this overrides the 'audio_encoder_ids' used during the live portion. If none are specified, the 'audio_encoder_ids' field will be used.
	FerAudioEncoderIds interface{} `json:"fer_audio_encoder_ids,omitempty" uniqueItems:"true" doc:"Optionally specify which audio encoders should be used when generating the FER of this Presentation, this overrides the 'audio_encoder_ids' used during the live portion. If none are specified, the 'audio_encoder_ids' field will be used."`
	Hls *PublicationHls `json:"hls,omitempty"`
	// List of video encoder IDs that should have I-Frame only playlists generated for them.
	IframeOnlyEncoderIds interface{} `json:"iframe_only_encoder_ids,omitempty" uniqueItems:"true" doc:"List of video encoder IDs that should have I-Frame only playlists generated for them."`
	// Optional master manifest name. When not supplied a default of 'master' will be used.
	MasterPlaylistName interface{} `json:"master_playlist_name,omitempty" doc:"Optional master manifest name. When not supplied a default of 'master' will be used."`
	Origin *PublicationOrigin `json:"origin,omitempty"`
	// Determines how segments in this publication are packaged. Must reference a packager in 'packaging.packagers'. However, if this is a playlist-only publication (i.e. contains publish points that specify 'playlist_only_for'), this must remain unset as the packager will be inferred from the publication this one is providing playlists for.
	PackagerId interface{} `json:"packager_id,omitempty" doc:"Determines how segments in this publication are packaged. Must reference a packager in 'packaging.packagers'. However, if this is a playlist-only publication (i.e. contains publish points that specify 'playlist_only_for'), this must remain unset as the packager will be inferred from the publication this one is providing playlists for."`
	// Publish points specify where to output.
	PublishPoints interface{} `json:"publish_points,omitempty" doc:"Publish points specify where to output."`
	// When redundant publishing is enabled succeeding to publish a given media segment to at least one HTTPPublishPoint in publish_points will result in that segment showing up in manifests as playable content. Will require at least two publish_points defined within the same publication.
	RedundantPublishing interface{} `json:"redundant_publishing,omitempty" doc:"When redundant publishing is enabled succeeding to publish a given media segment to at least one HTTPPublishPoint in publish_points will result in that segment showing up in manifests as playable content. Will require at least two publish_points defined within the same publication."`
	Startover *PublicationStartover `json:"startover,omitempty"`
	// Optional: Specify what thumbnail_encoders should be in this Publication
	ThumbnailEncoderIds interface{} `json:"thumbnail_encoder_ids,omitempty" uniqueItems:"true" doc:"Optional: Specify what thumbnail_encoders should be in this Publication"`
	// Optional, indicates whether we should pad the bitrate (false) or use what is explicitly provided (true)
	UseStrictBitrate interface{} `json:"use_strict_bitrate,omitempty" doc:"Optional, indicates whether we should pad the bitrate (false) or use what is explicitly provided (true)"`
	// Optionally specify which video encoders should be used for this publication. If none are specified, all video encoders configured for the transcoder will be used.
	VideoEncoderIds interface{} `json:"video_encoder_ids,omitempty" uniqueItems:"true" doc:"Optionally specify which video encoders should be used for this publication. If none are specified, all video encoders configured for the transcoder will be used."`
}

// NewPublication instantiates a new Publication object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPublication() *Publication {
	this := Publication{}
	return &this
}

// NewPublicationWithDefaults instantiates a new Publication object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPublicationWithDefaults() *Publication {
	this := Publication{}
	return &this
}

// GetAudioEncoderIds returns the AudioEncoderIds field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *Publication) GetAudioEncoderIds() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.AudioEncoderIds
}

// GetAudioEncoderIdsOk returns a tuple with the AudioEncoderIds field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *Publication) GetAudioEncoderIdsOk() (*interface{}, bool) {
	if o == nil || IsNil(o.AudioEncoderIds) {
		return nil, false
	}
	return &o.AudioEncoderIds, true
}

// HasAudioEncoderIds returns a boolean if a field has been set.
func (o *Publication) HasAudioEncoderIds() bool {
	if o != nil && IsNil(o.AudioEncoderIds) {
		return true
	}

	return false
}

// SetAudioEncoderIds gets a reference to the given interface{} and assigns it to the AudioEncoderIds field.
func (o *Publication) SetAudioEncoderIds(v interface{}) {
	o.AudioEncoderIds = v
}

// GetCreateVods returns the CreateVods field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *Publication) GetCreateVods() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.CreateVods
}

// GetCreateVodsOk returns a tuple with the CreateVods field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *Publication) GetCreateVodsOk() (*interface{}, bool) {
	if o == nil || IsNil(o.CreateVods) {
		return nil, false
	}
	return &o.CreateVods, true
}

// HasCreateVods returns a boolean if a field has been set.
func (o *Publication) HasCreateVods() bool {
	if o != nil && IsNil(o.CreateVods) {
		return true
	}

	return false
}

// SetCreateVods gets a reference to the given interface{} and assigns it to the CreateVods field.
func (o *Publication) SetCreateVods(v interface{}) {
	o.CreateVods = v
}

// GetDash returns the Dash field value if set, zero value otherwise.
func (o *Publication) GetDash() PublicationDash {
	if o == nil || IsNil(o.Dash) {
		var ret PublicationDash
		return ret
	}
	return *o.Dash
}

// GetDashOk returns a tuple with the Dash field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Publication) GetDashOk() (*PublicationDash, bool) {
	if o == nil || IsNil(o.Dash) {
		return nil, false
	}
	return o.Dash, true
}

// HasDash returns a boolean if a field has been set.
func (o *Publication) HasDash() bool {
	if o != nil && !IsNil(o.Dash) {
		return true
	}

	return false
}

// SetDash gets a reference to the given PublicationDash and assigns it to the Dash field.
func (o *Publication) SetDash(v PublicationDash) {
	o.Dash = &v
}

// GetDrms returns the Drms field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *Publication) GetDrms() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.Drms
}

// GetDrmsOk returns a tuple with the Drms field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *Publication) GetDrmsOk() (*interface{}, bool) {
	if o == nil || IsNil(o.Drms) {
		return nil, false
	}
	return &o.Drms, true
}

// HasDrms returns a boolean if a field has been set.
func (o *Publication) HasDrms() bool {
	if o != nil && IsNil(o.Drms) {
		return true
	}

	return false
}

// SetDrms gets a reference to the given interface{} and assigns it to the Drms field.
func (o *Publication) SetDrms(v interface{}) {
	o.Drms = v
}

// GetDvrWindowSecs returns the DvrWindowSecs field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *Publication) GetDvrWindowSecs() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.DvrWindowSecs
}

// GetDvrWindowSecsOk returns a tuple with the DvrWindowSecs field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *Publication) GetDvrWindowSecsOk() (*interface{}, bool) {
	if o == nil || IsNil(o.DvrWindowSecs) {
		return nil, false
	}
	return &o.DvrWindowSecs, true
}

// HasDvrWindowSecs returns a boolean if a field has been set.
func (o *Publication) HasDvrWindowSecs() bool {
	if o != nil && IsNil(o.DvrWindowSecs) {
		return true
	}

	return false
}

// SetDvrWindowSecs gets a reference to the given interface{} and assigns it to the DvrWindowSecs field.
func (o *Publication) SetDvrWindowSecs(v interface{}) {
	o.DvrWindowSecs = v
}

// GetFerAudioEncoderIds returns the FerAudioEncoderIds field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *Publication) GetFerAudioEncoderIds() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.FerAudioEncoderIds
}

// GetFerAudioEncoderIdsOk returns a tuple with the FerAudioEncoderIds field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *Publication) GetFerAudioEncoderIdsOk() (*interface{}, bool) {
	if o == nil || IsNil(o.FerAudioEncoderIds) {
		return nil, false
	}
	return &o.FerAudioEncoderIds, true
}

// HasFerAudioEncoderIds returns a boolean if a field has been set.
func (o *Publication) HasFerAudioEncoderIds() bool {
	if o != nil && IsNil(o.FerAudioEncoderIds) {
		return true
	}

	return false
}

// SetFerAudioEncoderIds gets a reference to the given interface{} and assigns it to the FerAudioEncoderIds field.
func (o *Publication) SetFerAudioEncoderIds(v interface{}) {
	o.FerAudioEncoderIds = v
}

// GetHls returns the Hls field value if set, zero value otherwise.
func (o *Publication) GetHls() PublicationHls {
	if o == nil || IsNil(o.Hls) {
		var ret PublicationHls
		return ret
	}
	return *o.Hls
}

// GetHlsOk returns a tuple with the Hls field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Publication) GetHlsOk() (*PublicationHls, bool) {
	if o == nil || IsNil(o.Hls) {
		return nil, false
	}
	return o.Hls, true
}

// HasHls returns a boolean if a field has been set.
func (o *Publication) HasHls() bool {
	if o != nil && !IsNil(o.Hls) {
		return true
	}

	return false
}

// SetHls gets a reference to the given PublicationHls and assigns it to the Hls field.
func (o *Publication) SetHls(v PublicationHls) {
	o.Hls = &v
}

// GetIframeOnlyEncoderIds returns the IframeOnlyEncoderIds field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *Publication) GetIframeOnlyEncoderIds() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.IframeOnlyEncoderIds
}

// GetIframeOnlyEncoderIdsOk returns a tuple with the IframeOnlyEncoderIds field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *Publication) GetIframeOnlyEncoderIdsOk() (*interface{}, bool) {
	if o == nil || IsNil(o.IframeOnlyEncoderIds) {
		return nil, false
	}
	return &o.IframeOnlyEncoderIds, true
}

// HasIframeOnlyEncoderIds returns a boolean if a field has been set.
func (o *Publication) HasIframeOnlyEncoderIds() bool {
	if o != nil && IsNil(o.IframeOnlyEncoderIds) {
		return true
	}

	return false
}

// SetIframeOnlyEncoderIds gets a reference to the given interface{} and assigns it to the IframeOnlyEncoderIds field.
func (o *Publication) SetIframeOnlyEncoderIds(v interface{}) {
	o.IframeOnlyEncoderIds = v
}

// GetMasterPlaylistName returns the MasterPlaylistName field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *Publication) GetMasterPlaylistName() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.MasterPlaylistName
}

// GetMasterPlaylistNameOk returns a tuple with the MasterPlaylistName field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *Publication) GetMasterPlaylistNameOk() (*interface{}, bool) {
	if o == nil || IsNil(o.MasterPlaylistName) {
		return nil, false
	}
	return &o.MasterPlaylistName, true
}

// HasMasterPlaylistName returns a boolean if a field has been set.
func (o *Publication) HasMasterPlaylistName() bool {
	if o != nil && IsNil(o.MasterPlaylistName) {
		return true
	}

	return false
}

// SetMasterPlaylistName gets a reference to the given interface{} and assigns it to the MasterPlaylistName field.
func (o *Publication) SetMasterPlaylistName(v interface{}) {
	o.MasterPlaylistName = v
}

// GetOrigin returns the Origin field value if set, zero value otherwise.
func (o *Publication) GetOrigin() PublicationOrigin {
	if o == nil || IsNil(o.Origin) {
		var ret PublicationOrigin
		return ret
	}
	return *o.Origin
}

// GetOriginOk returns a tuple with the Origin field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Publication) GetOriginOk() (*PublicationOrigin, bool) {
	if o == nil || IsNil(o.Origin) {
		return nil, false
	}
	return o.Origin, true
}

// HasOrigin returns a boolean if a field has been set.
func (o *Publication) HasOrigin() bool {
	if o != nil && !IsNil(o.Origin) {
		return true
	}

	return false
}

// SetOrigin gets a reference to the given PublicationOrigin and assigns it to the Origin field.
func (o *Publication) SetOrigin(v PublicationOrigin) {
	o.Origin = &v
}

// GetPackagerId returns the PackagerId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *Publication) GetPackagerId() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.PackagerId
}

// GetPackagerIdOk returns a tuple with the PackagerId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *Publication) GetPackagerIdOk() (*interface{}, bool) {
	if o == nil || IsNil(o.PackagerId) {
		return nil, false
	}
	return &o.PackagerId, true
}

// HasPackagerId returns a boolean if a field has been set.
func (o *Publication) HasPackagerId() bool {
	if o != nil && IsNil(o.PackagerId) {
		return true
	}

	return false
}

// SetPackagerId gets a reference to the given interface{} and assigns it to the PackagerId field.
func (o *Publication) SetPackagerId(v interface{}) {
	o.PackagerId = v
}

// GetPublishPoints returns the PublishPoints field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *Publication) GetPublishPoints() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.PublishPoints
}

// GetPublishPointsOk returns a tuple with the PublishPoints field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *Publication) GetPublishPointsOk() (*interface{}, bool) {
	if o == nil || IsNil(o.PublishPoints) {
		return nil, false
	}
	return &o.PublishPoints, true
}

// HasPublishPoints returns a boolean if a field has been set.
func (o *Publication) HasPublishPoints() bool {
	if o != nil && IsNil(o.PublishPoints) {
		return true
	}

	return false
}

// SetPublishPoints gets a reference to the given interface{} and assigns it to the PublishPoints field.
func (o *Publication) SetPublishPoints(v interface{}) {
	o.PublishPoints = v
}

// GetRedundantPublishing returns the RedundantPublishing field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *Publication) GetRedundantPublishing() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.RedundantPublishing
}

// GetRedundantPublishingOk returns a tuple with the RedundantPublishing field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *Publication) GetRedundantPublishingOk() (*interface{}, bool) {
	if o == nil || IsNil(o.RedundantPublishing) {
		return nil, false
	}
	return &o.RedundantPublishing, true
}

// HasRedundantPublishing returns a boolean if a field has been set.
func (o *Publication) HasRedundantPublishing() bool {
	if o != nil && IsNil(o.RedundantPublishing) {
		return true
	}

	return false
}

// SetRedundantPublishing gets a reference to the given interface{} and assigns it to the RedundantPublishing field.
func (o *Publication) SetRedundantPublishing(v interface{}) {
	o.RedundantPublishing = v
}

// GetStartover returns the Startover field value if set, zero value otherwise.
func (o *Publication) GetStartover() PublicationStartover {
	if o == nil || IsNil(o.Startover) {
		var ret PublicationStartover
		return ret
	}
	return *o.Startover
}

// GetStartoverOk returns a tuple with the Startover field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Publication) GetStartoverOk() (*PublicationStartover, bool) {
	if o == nil || IsNil(o.Startover) {
		return nil, false
	}
	return o.Startover, true
}

// HasStartover returns a boolean if a field has been set.
func (o *Publication) HasStartover() bool {
	if o != nil && !IsNil(o.Startover) {
		return true
	}

	return false
}

// SetStartover gets a reference to the given PublicationStartover and assigns it to the Startover field.
func (o *Publication) SetStartover(v PublicationStartover) {
	o.Startover = &v
}

// GetThumbnailEncoderIds returns the ThumbnailEncoderIds field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *Publication) GetThumbnailEncoderIds() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.ThumbnailEncoderIds
}

// GetThumbnailEncoderIdsOk returns a tuple with the ThumbnailEncoderIds field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *Publication) GetThumbnailEncoderIdsOk() (*interface{}, bool) {
	if o == nil || IsNil(o.ThumbnailEncoderIds) {
		return nil, false
	}
	return &o.ThumbnailEncoderIds, true
}

// HasThumbnailEncoderIds returns a boolean if a field has been set.
func (o *Publication) HasThumbnailEncoderIds() bool {
	if o != nil && IsNil(o.ThumbnailEncoderIds) {
		return true
	}

	return false
}

// SetThumbnailEncoderIds gets a reference to the given interface{} and assigns it to the ThumbnailEncoderIds field.
func (o *Publication) SetThumbnailEncoderIds(v interface{}) {
	o.ThumbnailEncoderIds = v
}

// GetUseStrictBitrate returns the UseStrictBitrate field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *Publication) GetUseStrictBitrate() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.UseStrictBitrate
}

// GetUseStrictBitrateOk returns a tuple with the UseStrictBitrate field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *Publication) GetUseStrictBitrateOk() (*interface{}, bool) {
	if o == nil || IsNil(o.UseStrictBitrate) {
		return nil, false
	}
	return &o.UseStrictBitrate, true
}

// HasUseStrictBitrate returns a boolean if a field has been set.
func (o *Publication) HasUseStrictBitrate() bool {
	if o != nil && IsNil(o.UseStrictBitrate) {
		return true
	}

	return false
}

// SetUseStrictBitrate gets a reference to the given interface{} and assigns it to the UseStrictBitrate field.
func (o *Publication) SetUseStrictBitrate(v interface{}) {
	o.UseStrictBitrate = v
}

// GetVideoEncoderIds returns the VideoEncoderIds field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *Publication) GetVideoEncoderIds() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.VideoEncoderIds
}

// GetVideoEncoderIdsOk returns a tuple with the VideoEncoderIds field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *Publication) GetVideoEncoderIdsOk() (*interface{}, bool) {
	if o == nil || IsNil(o.VideoEncoderIds) {
		return nil, false
	}
	return &o.VideoEncoderIds, true
}

// HasVideoEncoderIds returns a boolean if a field has been set.
func (o *Publication) HasVideoEncoderIds() bool {
	if o != nil && IsNil(o.VideoEncoderIds) {
		return true
	}

	return false
}

// SetVideoEncoderIds gets a reference to the given interface{} and assigns it to the VideoEncoderIds field.
func (o *Publication) SetVideoEncoderIds(v interface{}) {
	o.VideoEncoderIds = v
}

func (o Publication) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o Publication) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.AudioEncoderIds != nil {
		toSerialize["audio_encoder_ids"] = o.AudioEncoderIds
	}
	if o.CreateVods != nil {
		toSerialize["create_vods"] = o.CreateVods
	}
	if !IsNil(o.Dash) {
		toSerialize["dash"] = o.Dash
	}
	if o.Drms != nil {
		toSerialize["drms"] = o.Drms
	}
	if o.DvrWindowSecs != nil {
		toSerialize["dvr_window_secs"] = o.DvrWindowSecs
	}
	if o.FerAudioEncoderIds != nil {
		toSerialize["fer_audio_encoder_ids"] = o.FerAudioEncoderIds
	}
	if !IsNil(o.Hls) {
		toSerialize["hls"] = o.Hls
	}
	if o.IframeOnlyEncoderIds != nil {
		toSerialize["iframe_only_encoder_ids"] = o.IframeOnlyEncoderIds
	}
	if o.MasterPlaylistName != nil {
		toSerialize["master_playlist_name"] = o.MasterPlaylistName
	}
	if !IsNil(o.Origin) {
		toSerialize["origin"] = o.Origin
	}
	if o.PackagerId != nil {
		toSerialize["packager_id"] = o.PackagerId
	}
	if o.PublishPoints != nil {
		toSerialize["publish_points"] = o.PublishPoints
	}
	if o.RedundantPublishing != nil {
		toSerialize["redundant_publishing"] = o.RedundantPublishing
	}
	if !IsNil(o.Startover) {
		toSerialize["startover"] = o.Startover
	}
	if o.ThumbnailEncoderIds != nil {
		toSerialize["thumbnail_encoder_ids"] = o.ThumbnailEncoderIds
	}
	if o.UseStrictBitrate != nil {
		toSerialize["use_strict_bitrate"] = o.UseStrictBitrate
	}
	if o.VideoEncoderIds != nil {
		toSerialize["video_encoder_ids"] = o.VideoEncoderIds
	}
	return toSerialize, nil
}

type NullablePublication struct {
	value *Publication
	isSet bool
}

func (v NullablePublication) Get() *Publication {
	return v.value
}

func (v *NullablePublication) Set(val *Publication) {
	v.value = val
	v.isSet = true
}

func (v NullablePublication) IsSet() bool {
	return v.isSet
}

func (v *NullablePublication) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePublication(val *Publication) *NullablePublication {
	return &NullablePublication{value: val, isSet: true}
}

func (v NullablePublication) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePublication) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


