/*
 * WBD Aventus Channels API
 *
 * API version: 0.0.0
 * Contact: live-control-plane-devs@wbd.com
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package isp

import (
	"encoding/json"
)

// checks if the H265SettingsHdrDolbyVisionProfile81 type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &H265SettingsHdrDolbyVisionProfile81{}

// H265SettingsHdrDolbyVisionProfile81 struct for H265SettingsHdrDolbyVisionProfile81
type H265SettingsHdrDolbyVisionProfile81 struct {
	Clli *HdrSettingsContentLightLevelInfo `json:"clli,omitempty"`
	// Configures custom Level 8 metadata. Each target may be configured at most once. Unconfigured targets use the Dolby LDP SIDK defaults.
	Level8 interface{} `json:"level8,omitempty" doc:"Configures custom Level 8 metadata. Each target may be configured at most once. Unconfigured targets use the Dolby LDP SIDK defaults."`
	Mdcv *HdrSettingsMasteringDisplayColorVolume `json:"mdcv,omitempty"`
}

// NewH265SettingsHdrDolbyVisionProfile81 instantiates a new H265SettingsHdrDolbyVisionProfile81 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewH265SettingsHdrDolbyVisionProfile81() *H265SettingsHdrDolbyVisionProfile81 {
	this := H265SettingsHdrDolbyVisionProfile81{}
	return &this
}

// NewH265SettingsHdrDolbyVisionProfile81WithDefaults instantiates a new H265SettingsHdrDolbyVisionProfile81 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewH265SettingsHdrDolbyVisionProfile81WithDefaults() *H265SettingsHdrDolbyVisionProfile81 {
	this := H265SettingsHdrDolbyVisionProfile81{}
	return &this
}

// GetClli returns the Clli field value if set, zero value otherwise.
func (o *H265SettingsHdrDolbyVisionProfile81) GetClli() HdrSettingsContentLightLevelInfo {
	if o == nil || IsNil(o.Clli) {
		var ret HdrSettingsContentLightLevelInfo
		return ret
	}
	return *o.Clli
}

// GetClliOk returns a tuple with the Clli field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *H265SettingsHdrDolbyVisionProfile81) GetClliOk() (*HdrSettingsContentLightLevelInfo, bool) {
	if o == nil || IsNil(o.Clli) {
		return nil, false
	}
	return o.Clli, true
}

// HasClli returns a boolean if a field has been set.
func (o *H265SettingsHdrDolbyVisionProfile81) HasClli() bool {
	if o != nil && !IsNil(o.Clli) {
		return true
	}

	return false
}

// SetClli gets a reference to the given HdrSettingsContentLightLevelInfo and assigns it to the Clli field.
func (o *H265SettingsHdrDolbyVisionProfile81) SetClli(v HdrSettingsContentLightLevelInfo) {
	o.Clli = &v
}

// GetLevel8 returns the Level8 field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *H265SettingsHdrDolbyVisionProfile81) GetLevel8() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.Level8
}

// GetLevel8Ok returns a tuple with the Level8 field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *H265SettingsHdrDolbyVisionProfile81) GetLevel8Ok() (*interface{}, bool) {
	if o == nil || IsNil(o.Level8) {
		return nil, false
	}
	return &o.Level8, true
}

// HasLevel8 returns a boolean if a field has been set.
func (o *H265SettingsHdrDolbyVisionProfile81) HasLevel8() bool {
	if o != nil && IsNil(o.Level8) {
		return true
	}

	return false
}

// SetLevel8 gets a reference to the given interface{} and assigns it to the Level8 field.
func (o *H265SettingsHdrDolbyVisionProfile81) SetLevel8(v interface{}) {
	o.Level8 = v
}

// GetMdcv returns the Mdcv field value if set, zero value otherwise.
func (o *H265SettingsHdrDolbyVisionProfile81) GetMdcv() HdrSettingsMasteringDisplayColorVolume {
	if o == nil || IsNil(o.Mdcv) {
		var ret HdrSettingsMasteringDisplayColorVolume
		return ret
	}
	return *o.Mdcv
}

// GetMdcvOk returns a tuple with the Mdcv field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *H265SettingsHdrDolbyVisionProfile81) GetMdcvOk() (*HdrSettingsMasteringDisplayColorVolume, bool) {
	if o == nil || IsNil(o.Mdcv) {
		return nil, false
	}
	return o.Mdcv, true
}

// HasMdcv returns a boolean if a field has been set.
func (o *H265SettingsHdrDolbyVisionProfile81) HasMdcv() bool {
	if o != nil && !IsNil(o.Mdcv) {
		return true
	}

	return false
}

// SetMdcv gets a reference to the given HdrSettingsMasteringDisplayColorVolume and assigns it to the Mdcv field.
func (o *H265SettingsHdrDolbyVisionProfile81) SetMdcv(v HdrSettingsMasteringDisplayColorVolume) {
	o.Mdcv = &v
}

func (o H265SettingsHdrDolbyVisionProfile81) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o H265SettingsHdrDolbyVisionProfile81) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Clli) {
		toSerialize["clli"] = o.Clli
	}
	if o.Level8 != nil {
		toSerialize["level8"] = o.Level8
	}
	if !IsNil(o.Mdcv) {
		toSerialize["mdcv"] = o.Mdcv
	}
	return toSerialize, nil
}

type NullableH265SettingsHdrDolbyVisionProfile81 struct {
	value *H265SettingsHdrDolbyVisionProfile81
	isSet bool
}

func (v NullableH265SettingsHdrDolbyVisionProfile81) Get() *H265SettingsHdrDolbyVisionProfile81 {
	return v.value
}

func (v *NullableH265SettingsHdrDolbyVisionProfile81) Set(val *H265SettingsHdrDolbyVisionProfile81) {
	v.value = val
	v.isSet = true
}

func (v NullableH265SettingsHdrDolbyVisionProfile81) IsSet() bool {
	return v.isSet
}

func (v *NullableH265SettingsHdrDolbyVisionProfile81) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableH265SettingsHdrDolbyVisionProfile81(val *H265SettingsHdrDolbyVisionProfile81) *NullableH265SettingsHdrDolbyVisionProfile81 {
	return &NullableH265SettingsHdrDolbyVisionProfile81{value: val, isSet: true}
}

func (v NullableH265SettingsHdrDolbyVisionProfile81) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableH265SettingsHdrDolbyVisionProfile81) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


