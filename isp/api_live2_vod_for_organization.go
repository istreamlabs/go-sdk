...
If a channel with existing VODs is turned off or deleted, the VODs will still be returned.
	...
	*/
	...
	/*
	PostOrgChannelMp4 Archive an MP4 using Aventus Dynamic Manifest Origin.
	...
	@param org Organization name
	@param channelId Unique channel identifier
	@return ApiPostOrgChannelMp4Request
	*/
	PostOrgChannelMp4(ctx context.Context, org string, channelId string) ApiPostOrgChannelMp4Request

	// PostOrgChannelMp4Execute executes the request
	//  @return DynamicArchiveMP4Response
	PostOrgChannelMp4Execute(r ApiPostOrgChannelMp4Request) (*DynamicArchiveMP4Response, *http.Response, error)
...
type ApiPostOrgChannelMp4Request struct {
	ctx context.Context
	ApiService Live2VODForOrganizationApi
	org string
	channelId string
	dynamicArchiveMP4Request *DynamicArchiveMP4Request
}

func (r ApiPostOrgChannelMp4Request) DynamicArchiveMP4Request(dynamicArchiveMP4Request DynamicArchiveMP4Request) ApiPostOrgChannelMp4Request {
	r.dynamicArchiveMP4Request = &dynamicArchiveMP4Request
	return r
}

func (r ApiPostOrgChannelMp4Request) Execute() (*DynamicArchiveMP4Response, *http.Response, error) {
	return r.ApiService.PostOrgChannelMp4Execute(r)
}
...
Uses archive settings from the request to archive a FER
 ...
 @return ApiPostOrgChannelMp4Request
*/
func (a *Live2VODForOrganizationApiService) PostOrgChannelMp4(ctx context.Context, org string, channelId string) ApiPostOrgChannelMp4Request {
	return ApiPostOrgChannelMp4Request{
		ApiService: a,
		ctx: ctx,
		org: org,
...
// Execute executes the request
//  @return DynamicArchiveMP4Response
func (a *Live2VODForOrganizationApiService) PostOrgChannelMp4Execute(r ApiPostOrgChannelMp4Request) (*DynamicArchiveMP4Response, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPut
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *DynamicArchiveMP4Response
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "Live2VODForOrganizationApiService.PostOrgChannelMp4")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/v2/{org}/channels/{channel-id}/mp4"
	localVarPath = strings.Replace(localVarPath, "{"+"org"+"}", url.PathEscape(parameterToString(r.org, "")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"channel-id"+"}", url.PathEscape(parameterToString(r.channelId, "")), -1)
	...
	// body params
	localVarPostBody = r.dynamicArchiveMP4Request
	req, err := a.client.prepareRequest(r.ctx, localVarPath, localVarHTTPMethod, localVarPostBody, localVarHeaderParams, localVarQueryParams, localVarFormParams, formFiles)
	if err != nil {
		return localVarReturnValue, nil, err
	}
	...
	if disablePaging := r.ctx.Value(ContextDisablePaging); disablePaging == nil {
		if uri := GetLink(localVarHTTPResponse, RelNext); uri != nil {
			...
			localVarReturnValue = items.(*DynamicArchiveMP4Response)
			localVarHTTPResponse = resp
		}
	}
