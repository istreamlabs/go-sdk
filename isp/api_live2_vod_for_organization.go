...
type Live2VODForOrganizationAPI interface {
	...
	*/
	...
	/*
		PutOrgChannelQcu Create a QCU reel

		Creates a QCU task to assemble a playable reel from a list of moments

		@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
		@param org Organization name
		@param channelId Unique channel identifier
		@return ApiPutOrgChannelQcuRequest
	*/
	PutOrgChannelQcu(ctx context.Context, org string, channelId string) ApiPutOrgChannelQcuRequest

	// PutOrgChannelQcuExecute executes the request
	//  @return PutQCUResponse
	PutOrgChannelQcuExecute(r ApiPutOrgChannelQcuRequest) (*PutQCUResponse, *http.Response, error)
}
...
type ApiPutOrgChannelQcuRequest struct {
	ctx           context.Context
	ApiService    Live2VODForOrganizationAPI
	org           string
	channelId     string
	putQCURequest *PutQCURequest
}

func (r ApiPutOrgChannelQcuRequest) PutQCURequest(putQCURequest PutQCURequest) ApiPutOrgChannelQcuRequest {
	r.putQCURequest = &putQCURequest
	return r
}

func (r ApiPutOrgChannelQcuRequest) Execute() (*PutQCUResponse, *http.Response, error) {
	return r.ApiService.PutOrgChannelQcuExecute(r)
}
...
Creates a QCU task to assemble a playable reel from a list of moments
	...
	@return ApiPutOrgChannelQcuRequest
*/
func (a *Live2VODForOrganizationAPIService) PutOrgChannelQcu(ctx context.Context, org string, channelId string) ApiPutOrgChannelQcuRequest {
	return ApiPutOrgChannelQcuRequest{
		ApiService: a,
		ctx:        ctx,
		org:        org,
...
//
//	@return PutQCUResponse
func (a *Live2VODForOrganizationAPIService) PutOrgChannelQcuExecute(r ApiPutOrgChannelQcuRequest) (*PutQCUResponse, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodPut
		localVarPostBody    interface{}
		formFiles           []formFile
		localVarReturnValue *PutQCUResponse
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "Live2VODForOrganizationAPIService.PutOrgChannelQcu")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/v2/{org}/channels/{channel-id}/qcu"
	localVarPath = strings.Replace(localVarPath, "{"+"org"+"}", url.PathEscape(parameterToString(r.org, "")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"channel-id"+"}", url.PathEscape(parameterToString(r.channelId, "")), -1)
	...
	if r.putQCURequest == nil {
		return localVarReturnValue, nil, reportError("putQCURequest is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.putQCURequest
	req, err := a.client.prepareRequest(r.ctx, localVarPath, localVarHTTPMethod, localVarPostBody, localVarHeaderParams, localVarQueryParams, localVarFormParams, formFiles)
	if err != nil {
		return localVarReturnValue, nil, err
	}
	...
	if disablePaging := r.ctx.Value(ContextDisablePaging); disablePaging == nil {
		if uri := GetLink(localVarHTTPResponse, RelNext); uri != nil {
			...
			localVarReturnValue = items.(*PutQCUResponse)
			localVarHTTPResponse = resp
		}
	}
