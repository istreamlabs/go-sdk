/*
 * WBD Aventus Channels API
 *
 * API version: 0.0.0
 * Contact: live-control-plane-devs@wbd.com
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package isp

import (
	"encoding/json"
)

// checks if the PublishPoint type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PublishPoint{}

// PublishPoint struct for PublishPoint
type PublishPoint struct {
	// HTTP destination where media segments and playlists will be published.
	Http *PublishPointHTTP `json:"http,omitempty" doc:"HTTP destination where media segments and playlists will be published."`
	// uniquely identifies this publish_point within a channel configuration. Can be referenced by other publish_points in the 'playlist_only_for' field.
	Id interface{} `json:"id,omitempty" doc:"uniquely identifies this publish_point within a channel configuration. Can be referenced by other publish_points in the 'playlist_only_for' field."`
	// playlist_only_for identifies the id of the publish_point that has the segments for this publish_point, which is only publishing a playlist for those segments. All publish_points within a publication must either have 'playlist_only_for' all set or all not set.
	PlaylistOnlyFor interface{} `json:"playlist_only_for,omitempty" doc:"playlist_only_for identifies the id of the publish_point that has the segments for this publish_point, which is only publishing a playlist for those segments. All publish_points within a publication must either have 'playlist_only_for' all set or all not set."`
}

// NewPublishPoint instantiates a new PublishPoint object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPublishPoint() *PublishPoint {
	this := PublishPoint{}
	return &this
}

// NewPublishPointWithDefaults instantiates a new PublishPoint object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPublishPointWithDefaults() *PublishPoint {
	this := PublishPoint{}
	return &this
}

// GetHttp returns the Http field value if set, zero value otherwise.
func (o *PublishPoint) GetHttp() PublishPointHTTP {
	if o == nil || IsNil(o.Http) {
		var ret PublishPointHTTP
		return ret
	}
	return *o.Http
}

// GetHttpOk returns a tuple with the Http field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PublishPoint) GetHttpOk() (*PublishPointHTTP, bool) {
	if o == nil || IsNil(o.Http) {
		return nil, false
	}
	return o.Http, true
}

// HasHttp returns a boolean if a field has been set.
func (o *PublishPoint) HasHttp() bool {
	if o != nil && !IsNil(o.Http) {
		return true
	}

	return false
}

// SetHttp gets a reference to the given PublishPointHTTP and assigns it to the Http field.
func (o *PublishPoint) SetHttp(v PublishPointHTTP) {
	o.Http = &v
}

// GetId returns the Id field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *PublishPoint) GetId() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *PublishPoint) GetIdOk() (*interface{}, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return &o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *PublishPoint) HasId() bool {
	if o != nil && IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given interface{} and assigns it to the Id field.
func (o *PublishPoint) SetId(v interface{}) {
	o.Id = v
}

// GetPlaylistOnlyFor returns the PlaylistOnlyFor field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *PublishPoint) GetPlaylistOnlyFor() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.PlaylistOnlyFor
}

// GetPlaylistOnlyForOk returns a tuple with the PlaylistOnlyFor field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *PublishPoint) GetPlaylistOnlyForOk() (*interface{}, bool) {
	if o == nil || IsNil(o.PlaylistOnlyFor) {
		return nil, false
	}
	return &o.PlaylistOnlyFor, true
}

// HasPlaylistOnlyFor returns a boolean if a field has been set.
func (o *PublishPoint) HasPlaylistOnlyFor() bool {
	if o != nil && IsNil(o.PlaylistOnlyFor) {
		return true
	}

	return false
}

// SetPlaylistOnlyFor gets a reference to the given interface{} and assigns it to the PlaylistOnlyFor field.
func (o *PublishPoint) SetPlaylistOnlyFor(v interface{}) {
	o.PlaylistOnlyFor = v
}

func (o PublishPoint) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PublishPoint) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Http) {
		toSerialize["http"] = o.Http
	}
	if o.Id != nil {
		toSerialize["id"] = o.Id
	}
	if o.PlaylistOnlyFor != nil {
		toSerialize["playlist_only_for"] = o.PlaylistOnlyFor
	}
	return toSerialize, nil
}

type NullablePublishPoint struct {
	value *PublishPoint
	isSet bool
}

func (v NullablePublishPoint) Get() *PublishPoint {
	return v.value
}

func (v *NullablePublishPoint) Set(val *PublishPoint) {
	v.value = val
	v.isSet = true
}

func (v NullablePublishPoint) IsSet() bool {
	return v.isSet
}

func (v *NullablePublishPoint) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePublishPoint(val *PublishPoint) *NullablePublishPoint {
	return &NullablePublishPoint{value: val, isSet: true}
}

func (v NullablePublishPoint) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePublishPoint) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


